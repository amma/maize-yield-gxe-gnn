...
class GxETestDataset(Dataset):
    def __init__(self, trait, geno_reduced, env_scaled, g2i, e2i):
    ...
    def __getitem__(self, idx):
        ...
        graph = build_graph(self.geno_reduced[self.g2i[row["Hybrid"]]], self.env_scaled[self.e2i[row["Env"]]])
...
def main():
    parser = argparse.ArgumentParser(description="Test Architecture A")
    parser.add_argument("--model-path", default="results/arch_a/best_model.pt")
    ...
    parser.add_argument("--outdir", default="results/arch_a_test")
    ...
    loader = DataLoader(
        GxETestDataset(trait, geno_reduced, env_scaled, g2i, e2i),
